...
const AllBooks = () => {
  ...
  return (
    <div className='  bg-zinc-800 h-auto px-12 py-8'>
      <h3 className='text-3xl text-yellow-100'>All Books</h3>
      {!Data && (<div className='flex items-center justify-center'><Loader/></div> )}
      <div className='my-8 grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-4'>{Data && Data.map((items,i)=><div key={i}>
        <BookCard data={items}/>{" "}
      </div>)}</div>
    </div>
  )
